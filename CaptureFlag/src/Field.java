 ...
 * <p>
 * The field also acts as a moderator (umpire) for the game to try and prevent
 * players from cheating.
 * <p>
 * A field will have some width (x directions) and a height (y direction). The
 * upper left-hand corner is the [0,0] coordinate.
 * 
 * The values of x increase as you move to the right The values of y increase as
 * you move down
 */
public class Field {
	...
	/**
	 * distance between entities to be considered touching (from centre to
	 * centre)
	 */
	 ...
	 */
	public ArrayList<Entity> getTeam1() {
		return this.team1;
	}
	 ...
	 */
	public ArrayList<Entity> getTeam2() {
		return this.team2;
	}
	 ...
	 */
	public int[] getFlag1Position() {
		return new int[] { flag1.getX(), flag1.getY() };
	}
	 ...
	 */
	public int[] getFlag2Position() {
		return new int[] { flag2.getX(), flag2.getY() };
	}
	 ...
	 */
	public int[] getJail1Position() {
		return new int[] { jail1.getX(), jail1.getY() };
	}
	 ...
	 */
	public int[] getJail2Position() {
		return new int[] { jail2.getX(), jail2.getY() };
	}
	 ...
	 */
	public int[] getBase1Position() {
		return new int[] { base1.getX(), base1.getY() };
	}
	 ...
	 */
	public int[] getBase2Position() {
		return new int[] { base2.getX(), base2.getY() };
	}
	...
	public ArrayList<Entity> getThings() {
		/*
		 * hard-coded list of all things in the game. Change this if you want to
		 * add more things
		 */
		Entity[] allThings = new Entity[] { flag1, flag2, base1, base2, jail1,
				jail2 };
	 ...
	 * <p>
	 * Registers a player to a side (territory) on the field. If
	 * <code>START_FROM_BASE</code> is true, a registered player will have its
	 * position set be the same as the base for territory they are being
	 * registered to.
	 * <p>
	 * Side effects: registers a player with a territory. Assigns a sprite
	 * (image) to the player. Players position is set to their base if
	 * START_FROM_BASE is true.
	 * 
	 * @param a
	 *            is a player
	 * @param id
	 *            is the player's id number
	 * @param territory
	 *            is either 1 or 2 (representing a territory)
	 * @return true if <code>a</code> is not already registered,
	 *         <code>territory</code> is 1 or 2, and the player's coordinates
	 *         are not on the other territory. False, otherwise.
	 */
	 ...
	 * <p>
	 * Side effects: registers a non-player entity with a territory. Assigns a
	 * sprite (image) to the entity. Remembers the entity's id.
	 * 
	 * @param a
	 *            is an entity (non-player)
	 * @param id
	 *            is the entity's id number
	 * @param territory
	 *            is either 1 or 2 (representing a territory)
	 */
	 ...
	 * 
	 * @param a
	 *            is a player trying to catch player <code>b<\code>
	 * @param b
	 *            is a player
	 * @return true if <code>a</code> and <code>b</code> on are on different
	 *         teams and are within ARMS_LENGTH of each other. False otherwise.
	 */
	 ...
	 * 
	 * @param a
	 *            is a player trying to free a teammate from jail
	 * @param b
	 *            is a player
	 * @return true if <code>a</code> and <code>b</code> are on the same team,
	 *         <code<b</code> is in jail, and <code>a</code> is within
	 *         ARMS_REACH of the jail where <code>b</code> is located. False
	 *         otherwise.
	 */
	public boolean freeTeammate(Player a, Player b) {
		...
		int[] j = getJail1Position();
		if (getTeam.get(b).equals(new Integer(1))) {
			j = getJail2Position();
		}
		if (Math.hypot(a.getX() - j[0], a.getY() - j[1]) <= Field.ARMS_LENGTH) {
			return true;
		}
		return false;
	}
	 ...
	 * 
	 * @param a
	 *            is a player trying to pick up a flag
	 * @return true if <code>a</code> is within ARMS_REACH of the opposing
	 *         team's flag and it is not being carried by anyone else. Returns
	 *         false otherwise.
	 */
	 ...
	 * @param a
	 *            is a player
	 * @return true if <code>a</code> is carrying its opponent's flag and is
	 *         with ARMS_REACH of its own base. False otherwise.
	 */
	public boolean winGame() {
		for (Entity a : team1) {
			Entity b = base1;
			if (!((Player) a).pickUpFlag(this)) {
				continue;
			}
			if (Math.hypot(a.getX() - b.getX(), a.getY() - b.getY()) <= ARMS_LENGTH) {
				GAME_IS_WON = true;
	 ...
	 * 
	 * @return true if the game is still running (the game has not been won) and
	 *         false otherwise.
	 */
	...
	public boolean isInBase(Entity a) {
		int b[] = getBase2Position();
		if (getTeam.get(a).equals(new Integer(1))) {
			b = getBase1Position();
		}
		return Math.hypot(a.getX() - b[0], a.getY() - b[1]) <= ARMS_LENGTH;
	}
	
	/**
	 * create the default playing field
	 * <p>
	 * width of field = 800 pixels height of field = 600 pixels
	 */
	public Field() {
		...
		flag1 = new Flag(this, 1, 'f', 34 + minX, 191 + minY,
				"sprites/blueFlag.png");
		jail1 = new Jail(this, 1, 'j', 29 + minX, 399 + minY,
				"sprites/jail.png");
		base1 = new Base(this, 1, 'b', 29 + minX, 199 + minY,
				"sprites/blueBase.png");
		getTeam.put(base1, 1);
		getTeam.put(jail1, 1);
		getTeam.put(flag1, 1);
		things.add(base1);
		...
		flag2 = new Flag(this, 2, 'F', 754 + minX, 391 + minY,
				"sprites/redFlag.png");
		jail2 = new Jail(this, 2, 'J', 749 + minX, 199 + minY,
				"sprites/jail.png");
		base2 = new Base(this, 2, 'B', 749 + minX, 399 + minY,
				"sprites/redBase.png");
		getTeam.put(base2, 2);
		getTeam.put(jail2, 2);
		getTeam.put(flag2, 2);
