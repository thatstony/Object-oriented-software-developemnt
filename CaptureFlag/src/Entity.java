/**
 * Entity class to represent "things" in the game: players, flags, jails, bases,
 * etc.
 */
public abstract class Entity {
	...
	protected boolean isBeingFreed = false;
	...
	/**
	 * creates an entity (for players) with specified symbol at specified
	 * position
	 * 
	 * @param symbol
	 *            is a character (char) representation of the entity
	 * @param x
	 ...
	 * <p>
	 * In each direction, we have new_position = current_position + speed *
	 * time_elapsed
	 * <p>
	 * Note: TIME_SCALE will need to be manually adjusted to a value that allows
	 * good game play
	 * 
	 * @param time
	 *            is a length of time
	 ...
	 * @param id
	 *            is supposed to the entity's id value
	 */
	 ...
	 * 
	 * @param field
	 *            is the field that this entity is on
	 */
	protected void checkCoordinates(Field field)
	 ...
	 * 
	 * @param o
	 *            is an object to be tested for equality with this
	 ...
	 * Setter methods
	 * 
	 * We don't want a player of one team to be able to change the position or
	 * movement of a player on another team. The caller needs to know the
	 * entity's id number.
	 */
	...
	protected final void setY(double y, int id) {
		...
		if (id != this.id) {
			throw new SecurityException(
					"Unauthorized change of entity y coordinate");
		}
		this.y = y;
	}
	...
	protected final void setSpeedX(double speedX, int id) {
		...
		if (id != this.id) {
			throw new SecurityException(
					"Unauthorized change of entity x-direction speed");
		}
		this.speedX = speedX;
	}
	...
	protected final void setSpeedY(double speedY, int id) {
		...
		if (id != this.id) {
			throw new SecurityException(
					"Unauthorized change of entity y-direction speed");
		}
		this.speedY = speedY;
	}
	...
	public void setSprite(String ref, int id) {
		...
		if (id != this.id) {
			throw new SecurityException(
					"Unauthorized change of entity's sprite");
		}
		this.sprite = SpriteStore.get().getSprite(ref);
	}
	...
	public boolean isBeingFreed() {
		return isBeingFreed;
	}

	public void setBeingFreed(boolean isBeingFreed) {
		this.isBeingFreed = isBeingFreed;
	}
